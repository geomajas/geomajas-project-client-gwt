 ...
 */
@Api
public abstract class AbstractAttributeSearchPanel extends AbstractSearchPanel {

	protected static final SearchAndFilterMessages MESSAGES = GWT.create(SearchAndFilterMessages.class);

	protected static final String STYLE_SEARCH_ROW = "searchRow";
	protected static final String STYLE_SEARCH_HEADER = "searchHeader";
	protected static final String STYLE_HEADER_BAR = "headerBar";

	protected AbstractFeatureSearch featureSearch;
	 ...
	 */
	public AbstractAttributeSearchPanel(final MapWidget mapWidget, boolean manualLayerSelection, String layerId) {
		super(mapWidget);
		featureSearch = createFeatureSearch(mapWidget, manualLayerSelection, layerId);
		addChild(featureSearch);
	}

	public abstract AbstractFeatureSearch createFeatureSearch(final MapWidget mapWidget, boolean manualLayerSelection,
															  String layerId);
	 ...
	 * @author Kristof Heirwegh
	 */
	public abstract class AbstractFeatureSearch extends Canvas {
		...
		private boolean includeServerLayerInfo; // is this related to manualLayerSelection?

		private boolean enableRulesCrud;
		 ...
		 */
		public AbstractFeatureSearch(MapModel mapModel, final String layerId, boolean manualLayerSelection,
									 boolean includeServerLayerInfo, boolean enableRulesCrud) {
			super();
		...
		public void setSearchCriteria(Criterion criterion) {
			if (criterion != null) {
				...
				for (int i = 0; i < criteria.size(); i++) {
					...
					if (includeServerLayerInfo && vl == null) {
				...
				if (enableRulesCrud && criteria.size() > 1) {
					buttonPanes.get(0).getMember(1).setDisabled(false);
				}
			}
		}
